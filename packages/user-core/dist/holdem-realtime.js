...
const HOLDEM_HERO_CARD_PLACEHOLDERS = [
    { rank: null, suit: null, hidden: true },
    { rank: null, suit: null, hidden: true },
];
const mapRevealedCards = (cards) => cards.map((card) => ({
    rank: card.rank,
    suit: card.suit,
    hidden: false,
}));
const clonePlaceholderCards = () => HOLDEM_HERO_CARD_PLACEHOLDERS.map((card) => ({ ...card }));
const buildOpenSeat = (seatIndex, table) => ({
    seatIndex,
    userId: null,
    displayName: null,
...
const patchSelectedHoldemTable = (params) => {
    ...
    const existingSeatsByIndex = new Map(currentTable.seats.map((seat) => [seat.seatIndex, seat]));
    const publicSeatsByIndex = new Map(publicTable.seats.map((seat) => [seat.seatIndex, seat]));
    const resetHeroCards = update.eventTypes.some((eventType) => HOLDEM_EVENT_TYPES_THAT_RESET_HERO_STATE.has(eventType));
...
const patchLobbyTableSummary = (params) => {
    ...
    const summary = {
        ...
        heroSeatIndex: tables.tables.find((table) => table.id === update.table.id)?.heroSeatIndex ?? null,
    ...
    const nextTables = tables.tables.some((table) => table.id === update.table.id)
        ? tables.tables.map((table) => table.id === update.table.id
            ? {
                ...table,
                ...summary,
            }
...
export const applyHoldemRealtimeUpdate = (params) => {
    const nextLobby = params.holdemTables === null
        ? null
        : patchLobbyTableSummary({
            tables: params.holdemTables,
            update: params.update,
        });
    const shouldPatchSelectedTable = params.selectedHoldemTable !== null &&
    ...
    return {
        nextLobby,
        nextTable: shouldPatchSelectedTable && params.selectedHoldemTable
...
export const applyHoldemPrivateRealtimeUpdate = (params) => params.selectedHoldemTableId === params.update.table.id
    ? {
        table: params.update.table,
    }
    : params.selectedHoldemTable;
export const applyHoldemTableMessage = (params) => {
    const dedupedMessages = params.currentMessages.filter((entry) => entry.id !== params.message.id);
    ...
    const maxMessages = Math.max(1, params.maxMessages ?? HOLDEM_TABLE_MESSAGE_LIMIT);
    return dedupedMessages.slice(-maxMessages);
};
export const createHoldemRealtimeClient = (params) => {
    let currentStatus = "connecting";
    let requestedTopics = new Set();
    ...
    const applyTopicDiff = () => {
        ...
        const topicsToSubscribe = nextTopics.filter((topic) => !subscribedTopics.has(topic));
        const topicsToUnsubscribe = currentTopics.filter((topic) => !requestedTopics.has(topic));
    ...
    const connect = async () => {
        ...
        try {
            ...
            params.onWarning?.(error instanceof Error ? error.message : "Failed to prepare realtime auth token.");
        ...
        nextSocket.onmessage = (event) => {
            ...
            if (parsed.data.type === "transport.error") {
                if (parsed.data.code === REALTIME_ERROR_CODES.RESUME_NOT_AVAILABLE ||
            ...
            const payload = HoldemRealtimePrivateUpdateSchema.safeParse(parsed.data.data);
            if (!payload.success) {
                return;
            }
    ...
    return {
        ...
        syncTopics(topics) {
            requestedTopics = new Set(topics.filter((topic) => topic.trim().length > 0));
            applyTopicDiff();
        },
    };
};
