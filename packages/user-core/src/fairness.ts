...
const encodeUtf8 = (value: string) => {
  ...
  return Array.from(unescape(encodeURIComponent(value))).map((char) =>
    char.charCodeAt(0),
  );
};
...
export function sha256Hex(input: string) {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 =
        (((((h + s1) | 0) + choice) | 0) + SHA256_K[index] + words[index]) >>>
        0;
...
export function verifyFairnessReveal(
  input: Pick<FairnessReveal, "seed" | "commitHash">,
...
export function getFairnessRevealDate(
  fairness: Pick<FairnessCommit, "epoch" | "epochSeconds"> | null | undefined,
) {
  if (!fairness) {
    return null;
  }
