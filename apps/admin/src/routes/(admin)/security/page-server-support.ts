...
type SecurityLoadEvent = Pick<
  RequestEvent,
  "fetch" | "cookies" | "locals" | "url"
>
type SecurityActionEvent = Pick<
  RequestEvent,
...
export const loadSecurityPage = async ({
  fetch,
  cookies,
  locals,
  url,
  ...
  try {
    ...
    if (
      !authEvents.success ||
      !freezeRecords.success ||
      !jurisdictionRules.success
    ) {
...
export const securityPageActions = {
  releaseFreeze: async ({
    request,
    fetch,
    cookies,
    ...
    if (!freezeRecordId) {
      return securityActionFail(400, t, "security.errors.missingFreezeRecordId")
    }
