...
type MarketsLoadEvent = Pick<RequestEvent, "fetch" | "cookies" | "locals">
type MarketsActionEvent = Pick<
  RequestEvent,
...
const buildOracleBindingPayload = (
  ...
  if (!provider) {
    return {
      oracleBinding: null,
      error: t("markets.errors.oracleProviderRequired"),
    }
  }
...
export const loadMarketsPage = async ({
  ...
  try {
    ...
    const markets = marketsResponse.ok
      ? PredictionMarketSummarySchema.array().safeParse(
...
export const marketsPageActions = {
  create: async ({ request, fetch, cookies, locals }: MarketsActionEvent) => {
    ...
    if (parsedOracleBinding.error || !parsedOracleBinding.oracleBinding) {
      return fail(400, {
        error:
          parsedOracleBinding.error ?? t("markets.errors.invalidOracleBinding"),
      })
    }
  ...
  settle: async ({ request, fetch, cookies, locals }: MarketsActionEvent) => {
    const t = getActionT(locals?.locale)
    const formData = await request.formData()
  ...
  cancel: async ({ request, fetch, cookies, locals }: MarketsActionEvent) => {
    const t = getActionT(locals?.locale)
    const formData = await request.formData()
    const marketId = parseRequiredText(formData.get("marketId"))
    const stepUpPayload = parseAdminStepUpPayload(formData)
  ...
  acknowledgeAppeal: async ({
    ...
    const parsed =
      PredictionMarketAppealAcknowledgeRequestSchema.safeParse(payload)
    if (!parsed.success) {
      return fail(400, {
        error:
          parsed.error.issues[0]?.message ??
