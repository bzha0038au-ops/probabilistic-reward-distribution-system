...
export const actions: Actions = {
  freezeScope: async ({ request, fetch, cookies, params, locals }) => {
    const t = createTranslator(getMessages(locals.locale))
    const userId = parseUserId(params.userId)
    if (!userId) {
    ...
    const response = await apiRequest(fetch, cookies, `/admin/users/${userId}/freeze`, {
      method: "POST",
  ...
  unfreezeScope: async ({ request, fetch, cookies, params, locals }) => {
    const t = createTranslator(getMessages(locals.locale))
    const userId = parseUserId(params.userId)
    if (!userId) {
  ...
  forceLogout: async ({ request, fetch, cookies, params, locals }) => {
    const t = createTranslator(getMessages(locals.locale))
    const userId = parseUserId(params.userId)
    if (!userId) {
  ...
  resetPassword: async ({ request, fetch, cookies, params, locals }) => {
    const t = createTranslator(getMessages(locals.locale))
    const userId = parseUserId(params.userId)
    if (!userId) {
