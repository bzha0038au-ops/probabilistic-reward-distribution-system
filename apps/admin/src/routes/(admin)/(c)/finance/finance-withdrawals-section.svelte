      ...
      <table class="table admin-table-compact">
        ...
        <tbody>
          {#each filteredWithdrawals as withdrawal}
            <tr>
              <td class="font-mono text-xs">{withdrawal.id}</td>
              <td class="font-mono text-xs">{withdrawal.userId}</td>
              <td class="font-mono text-xs">{withdrawal.amount}</td>
              <td>
                <div class="space-y-1">
                  <div>
                    {support.withdrawalStatusLabel(
                      withdrawal.status,
                      withdrawal.metadata,
                    )}
                  </div>
              ...
              <td
                >{withdrawal.payoutMethodId ?? withdrawal.bankCardId ?? "-"}</td
              >
              <td>
                <div class="space-y-1 text-xs">
                  ...
                  {#if support.ledgerStateLabel(Reflect.get(withdrawal.metadata ?? {}, "ledgerState"))}
                    <div class="text-slate-500">
                      {support.ledgerStateLabel(
                        Reflect.get(withdrawal.metadata ?? {}, "ledgerState"),
                      )}
                    </div>
                  {/if}
                  {#if readString(Reflect.get(withdrawal.metadata ?? {}, "failureReason"))}
                    <div class="text-slate-500">
                      Failure: {readString(
                        Reflect.get(withdrawal.metadata ?? {}, "failureReason"),
                      )}
                    </div>
                  {/if}
              ...
              <td>
                {#if support.formatReview(withdrawal.metadata)}
                  <div class="space-y-1 text-xs">
                    <div>
                      {support.formatReview(withdrawal.metadata)?.action ?? "-"}
                    </div>
                    {#if support.formatReview(withdrawal.metadata)?.adminId}
                      <div class="text-slate-500">
                        Admin #{support.formatReview(withdrawal.metadata)
                          ?.adminId}
                      </div>
                    {/if}
                    ...
                    {#if support.formatReview(withdrawal.metadata)?.processingChannel}
                      <div class="text-slate-500">
                        {support.formatReview(withdrawal.metadata)
                          ?.processingChannel}
                      </div>
                    {/if}
                ...
                {#if withdrawal.status === "requested" || withdrawal.status === "approved" || withdrawal.status === "provider_submitted" || withdrawal.status === "provider_processing" || withdrawal.status === "provider_failed" || withdrawal.status === "paid"}
                  <div class="admin-inline-actions">
                    {#if withdrawal.status === "requested"}
                      <form method="post" action="?/approveWithdrawal">
                        ...
                        <button class="btn btn-xs btn-primary" type="submit">
                          {support.withdrawalApproveActionLabel(
                            withdrawal.metadata,
                          )}
                        </button>
                      </form>
                    ...
                    {#if withdrawal.status === "approved"}
                      <form
                        method="post"
                        action={isCrypto(withdrawal.channelType)
                    ...
                    {#if withdrawal.status === "provider_submitted"}
                      ...
                      <form
                        method="post"
                        action="?/markWithdrawalProviderFailed"
                      >
                        <input type="hidden" name="id" value={withdrawal.id} />
                        <input
                          type="hidden"
