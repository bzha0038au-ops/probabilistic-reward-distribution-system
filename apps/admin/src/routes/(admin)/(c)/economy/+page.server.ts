...
export const actions: Actions = {
  replayFulfillment: async ({ request, fetch, cookies }) => {
    ...
    if (!response.ok) {
      return fail(response.status, {
        error:
          response.error?.message ?? "Failed to replay store purchase fulfillment.",
      })
    }
  ...
  reverseOrder: async ({ request, fetch, cookies }) => {
    const formData = await request.formData()
    const orderId = parsePositiveInt(formData.get("orderId"))
    ...
    if (!orderId || (targetStatus !== "refunded" && targetStatus !== "revoked") || !reason) {
    ...
    const response = await apiRequest(fetch, cookies, `/admin/economy/orders/${orderId}/reverse`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
    ...
    if (!response.ok) {
      return fail(response.status, {
        error: response.error?.message ?? "Failed to reverse store purchase order.",
      })
    }
  ...
  adjustAsset: async ({ request, fetch, cookies }) => {
    const formData = await request.formData()
    ...
    if (
      ...
      return invalidForm("User, asset, direction, amount, and reason are required.")
    }
    const stepUpError = validateAdminStepUpPayload(stepUp)
    if (stepUpError) {
      return invalidForm(stepUpError)
    ...
    const response = await apiRequest(fetch, cookies, "/admin/economy/adjustments", {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
  ...
  freezeGift: async ({ request, fetch, cookies }) => {
    const formData = await request.formData()
    const userId = parsePositiveInt(formData.get("userId"))
  ...
  releaseGiftFreeze: async ({ request, fetch, cookies }) => {
    const formData = await request.formData()
    ...
    const response = await apiRequest(fetch, cookies, `/admin/freeze-records/${freezeRecordId}/release`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
      },
