...
type KycDetailLoadEvent = Pick<
  RequestEvent,
  "fetch" | "cookies" | "params" | "locals"
>
type KycDetailActionEvent = Pick<
  RequestEvent,
  "request" | "fetch" | "cookies" | "params" | "locals"
>

const parseProfileId = (value: string | undefined) => {
...
export const loadKycDetailPage = async ({
  fetch,
  cookies,
  params,
  locals,
}: KycDetailLoadEvent) => {
  const copy = getKycCopy(locals.locale ?? "en")
...
const submitReviewAction = async (params: {
  ...
  const copy = getKycCopy(params.locale ?? "en")
...
export const kycDetailPageActions = {
  approve: async ({
    request,
    fetch,
    cookies,
    ...
    submitReviewAction({
      ...
      path: "/approve",
      successMessage: getKycCopy(locals.locale ?? "en").messages.approved,
      action: "approve",
    }),
  reject: async ({
    request,
    ...
    submitReviewAction({
      ...
      locale: locals.locale,
      path: "/reject",
      successMessage: getKycCopy(locals.locale ?? "en").messages.rejected,
      action: "reject",
      requireReason: true,
    }),
  requestMoreInfo: async ({
    ...
    submitReviewAction({
      ...
      successMessage: getKycCopy(locals.locale ?? "en").messages
        .moreInfoRequested,
      action: "requestMoreInfo",
    }),
  requestReverification: async ({
    request,
    fetch,
    cookies,
    params,
