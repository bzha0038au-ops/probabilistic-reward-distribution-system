  ...
  <div class="card-body space-y-4">
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.9fr)]">
      ...
      >
        ...
        <p class="mt-1 text-slate-700">
          这里的改动会直接作用在用户入口、支付路径、抽奖成本、以及 Blackjack
          规则。
        </p>
      </div>
    </div>

    <form method="post" action="?/configDraft" class="grid gap-6">
      ...
      >
        <div class="mb-4">
          ...
          <p class="mt-2 text-sm font-semibold">Blackjack 规则</p>
      ...
      <div class="border-t border-[var(--admin-border)] pt-5">
        ...
        >
          <p class="max-w-[32rem] text-xs leading-6 text-slate-500">
            保存的是草稿，不会直接写生产；但理由、操作者和 step-up
            状态都会进入审计链。
          </p>
...
<div class="card bg-base-100 shadow">
  <div class="card-body space-y-4">
    <div>
      ...
      <h2 class="mt-2 card-title">{t("admin.bonus.title")}</h2>
