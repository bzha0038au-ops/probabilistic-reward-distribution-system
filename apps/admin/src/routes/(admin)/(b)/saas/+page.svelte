<script lang="ts">
  ...
  import type { SaasApiKeyIssue } from "@reward/shared-types/saas"
  import { saasProjectStrategyValues } from "@reward/shared-types/saas"
  import { getContext } from "svelte"
  import { saasActionPolicies } from "./action-policies"
  import type {
    HelloRewardQuickstart,
    InviteResult,
    PageData,
    RiskEnvelopeDraftResult,
    TenantProvisioned,
  } from "./page-support"
  import SaasModuleTabs from "./saas-module-tabs.svelte"
  ...
  const tenantProvisioned = $derived(
    ($page.form?.tenantProvisioned as TenantProvisioned | undefined) ?? null,
  )
  ...
  const activeModule = $derived.by(() => {
    if ($page.url.pathname === "/saas/tenants") return "tenants"
    if ($page.url.pathname === "/saas/projects") return "projects"
    if ($page.url.pathname === "/saas/billing") return "billing"
    if ($page.url.pathname === "/saas/webhooks") return "webhooks"
    if ($page.url.pathname === "/saas/disputes") return "disputes"
    return "hub"
  })
  const isHubModule = $derived(activeModule === "hub")
  const isTenantsModule = $derived(activeModule === "tenants")
  const isProjectsModule = $derived(activeModule === "projects")
  const isBillingModule = $derived(activeModule === "billing")
  const isWebhooksModule = $derived(activeModule === "webhooks")
  ...
  const billableTenantSummaryCount = $derived(
    tenants.filter((item) => item.billing?.isBillable ?? false).length,
  )
  const tenantLinkCount = $derived(tenantLinks.length)
  const activeAgentControlCount = $derived(agentControls.length)
  const unsettledBillingRunCount = $derived(
    billingRuns.filter(
      (billingRun) =>
        !["settled", "synced"].includes(String(billingRun.status)),
    ).length,
  )
  const pendingTopUpCount = $derived(
    topUps.filter(
      (topUp) => !["applied", "synced"].includes(String(topUp.status)),
    ).length,
  )
  const billingSyncTenantCount = $derived(
    tenants.filter((item) => item.billing?.providerCapabilities.billingRunSync)
      .length,
  )
  ...
  const formatDecisionBreakdown = (
    ...
    breakdown
      ...
      .join(" · ")
  const saasModules = $derived([
    {
      href: "/saas/tenants",
      eyebrow: "Tenant Directory",
      title: "Tenants",
      description:
        "租户、membership、invite、agent tree 和风险包络集中到一页。",
    ...
    },
    {
      ...
      title: "Projects",
      description:
        "项目、API keys、hello-reward quickstart 和 prize registry 单独处理。",
    ...
    },
    {
      ...
      title: "Billing",
      description:
        "billing runs、manual credits、recent usage 与 disputes 分开处理。",
      badge: `${billingRuns.length}`,
    },
    {
      href: "/saas/webhooks",
      eyebrow: "Delivery Queue",
      title: "Webhooks",
      description: "outbound webhook、事件队列和 delivery 状态从主页抽离。",
      badge: `${outboundWebhooks.length}`,
    },
  ])
...
<div class="space-y-8" onsubmitcapture={handleBreakGlassSubmit}>
  <section class="space-y-3">
    ...
    <div class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow">
      <div>
    ...
    {#if data.inviteToken}
      <form
        method="post"
        action="?/acceptInvite"
        class="rounded-3xl border border-amber-300 bg-amber-50 p-5 shadow"
      >
  ...
  <section class="admin-summary-grid grid gap-4 md:grid-cols-3 xl:grid-cols-6">
    <article class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow">
  ...
  {#if isTenantsModule}
    <section
      class="admin-summary-grid grid gap-4 md:grid-cols-2 xl:grid-cols-4"
    >
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Tenant directory</p>
        <p class="mt-3 text-3xl font-semibold">{tenants.length}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Pending invites</p>
        <p class="mt-3 text-3xl font-semibold">{pendingInviteCount}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Agent links</p>
        <p class="mt-3 text-3xl font-semibold">{tenantLinkCount}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Billable tenants</p>
        <p class="mt-3 text-3xl font-semibold">{billableTenantSummaryCount}</p>
      </article>
    </section>
  {/if}
  ...
  {#if isBillingModule}
    ...
    >
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Billing runs</p>
        <p class="mt-3 text-3xl font-semibold">{billingRuns.length}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Actionable runs</p>
        <p class="mt-3 text-3xl font-semibold">{unsettledBillingRunCount}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Pending credits</p>
        <p class="mt-3 text-3xl font-semibold">{pendingTopUpCount}</p>
      </article>
      <article
        class="rounded-3xl border border-base-300 bg-base-100 p-5 shadow"
      >
        <p class="text-sm text-slate-500">Sync-enabled tenants</p>
        <p class="mt-3 text-3xl font-semibold">{billingSyncTenantCount}</p>
      </article>
    </section>
  {/if}
    ...
    <section class="card bg-base-100 shadow">
      <div class="card-body space-y-5">
        ...
        <p class="max-w-3xl text-sm leading-6 text-[var(--admin-muted)]">
          SaaS 根页现在只保留总览与模块入口。进入具体模块，再处理
          tenants、projects、billing 和 webhooks。
        </p>

        <div class="grid gap-4 lg:grid-cols-2 2xl:grid-cols-4">
          {#each saasModules as module}
            <a
        ...
        <div class="flex flex-wrap gap-3">
          <a class="btn btn-outline btn-sm" href="/saas/disputes">
            Open disputes
          </a>
        </div>
      </div>
  ...
  {#if isTenantsModule}
    <section class="grid gap-6 xl:grid-cols-3">
      <form
        method="post"
        action="?/createTenant"
        class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow"
      >
        ...
        <div>
          ...
          <h2 class="mt-2 text-lg font-semibold">
            {t("saas.tenantForm.title")}
          </h2>
          <p class="mt-2 text-sm text-slate-500">
            先建立租户壳，再进入 billing、project 和 usage 模块继续配置。
          </p>
        </div>
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        <div>
          <p
            class="font-mono text-[0.68rem] uppercase tracking-[0.22em] text-[var(--admin-primary)]"
          >
        ...
        <form method="post" action="?/assignMembership" class="mt-4 space-y-3">
          ...
          <select class="select select-bordered w-full" name="role">
            ...
            <option value="tenant_operator"
              >{translateRole("tenant_operator")}</option
            >
            <option value="agent_manager"
              >{translateRole("agent_manager")}</option
            >
        ...
        <div
          class="mb-3 rounded-2xl bg-base-200/50 px-4 py-3 text-xs text-slate-500"
        >
          {memberships.length} memberships · {pendingInviteCount} pending invites
        </div>
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        <div>
          ...
          <h2 class="mt-2 text-lg font-semibold">
            {t("saas.agentTree.title")}
          </h2>
          <p class="mt-2 text-sm text-slate-500">
            管理 parent / child 租户链路，单独处理 agent-client 编排关系。
          </p>
        </div>
        <form method="post" action="?/linkTenant" class="mt-4 space-y-3">
          <input type="hidden" name="totpCode" value={stepUpCode} />
        ...
        <div class="mt-5 grid gap-3 sm:grid-cols-2">
          <div class="rounded-2xl bg-base-200/50 p-4 text-sm">
            <p class="text-slate-500">Linked tenants</p>
            <p class="mt-2 text-2xl font-semibold text-base-content">
              {tenantLinkCount}
            </p>
          </div>
          <div class="rounded-2xl bg-base-200/50 p-4 text-sm">
            <p class="text-slate-500">Active controls</p>
            <p class="mt-2 text-2xl font-semibold text-base-content">
          ...
          <div class="mt-4 grid gap-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              <select class="select select-bordered w-full" name="environment">
                ...
                <option value="live"
                  >{translateProjectEnvironment("live")}</option
                >
              </select>
              <select class="select select-bordered w-full" name="status">
                <option value="active">{translateTenantStatus("active")}</option
                ...
                <option value="archived"
                  >{translateTenantStatus("archived")}</option
                >
              </select>
            </div>
            <div class="grid grid-cols-2 gap-3">
  ...
  {#if isBillingModule}
    <section class="grid gap-6 xl:grid-cols-3">
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        ...
        <form method="post" action="?/createTopUp" class="mt-4 space-y-3">
          <input type="hidden" name="totpCode" value={stepUpCode} />
          <select class="select select-bordered w-full" name="tenantId">
            {#each tenants as item}
              <option value={item.tenant.id}>{item.tenant.name}</option>
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        <div>
          ...
          <h2 class="mt-2 text-lg font-semibold">Settlement posture</h2>
          <p class="mt-2 text-sm text-slate-500">
            把 sync 能力、manual credit 模式和最近 usage
            放在一侧，避免和创建表单混在一屏。
          </p>
        </div>
        <div class="mt-4 grid gap-3">
          <div class="rounded-2xl bg-base-200/50 p-4 text-sm">
            <p class="text-slate-500">Invoice sync</p>
            <p class="mt-2 text-2xl font-semibold text-base-content">
              {billingSyncTenantCount}
            </p>
            <p class="mt-1 text-xs text-slate-500">
              tenants with external settlement hooks
            </p>
          </div>
          <div class="rounded-2xl bg-base-200/50 p-4 text-sm">
            <p class="text-slate-500">Manual credits</p>
            <p class="mt-2 text-2xl font-semibold text-base-content">
              {pendingTopUpCount}
            </p>
            <p class="mt-1 text-xs text-slate-500">
              {localManualCreditsAvailable
                ? "local credit flow enabled"
                : "top-up requests require sync or manual apply"}
            </p>
          </div>
          <div class="rounded-2xl bg-base-200/50 p-4 text-sm">
            <p class="text-slate-500">Recent usage events</p>
            <p class="mt-2 text-2xl font-semibold text-base-content">
              {recentUsage.length}
            </p>
            <p class="mt-1 text-xs text-slate-500">
              runtime usage remains visible below for billing triage
            </p>
          </div>
        </div>
      </div>
  ...
  {#if isTenantsModule || isProjectsModule}
    <section class="grid gap-6 xl:grid-cols-3">
      {#if isTenantsModule}
        <div
          class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow xl:col-span-2"
        >
          <h2 class="text-lg font-semibold">
            {t("saas.tenantsSection.title")}
          </h2>
            ...
            {#each tenants as item}
              <article class="rounded-2xl border border-base-300 p-4">
                ...
                >
                  ...
                  <div class="flex flex-col gap-2 lg:min-w-[18rem]">
                    ...
                    <form
                      method="post"
                      action="?/saveBilling"
                      class="grid gap-2 rounded-2xl bg-base-200/50 p-3"
                    >
                      <input type="hidden" name="totpCode" value={stepUpCode} />
                      <input
                        type="hidden"
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        ...
                        >
                          ...
                          <option
                            ...
                            value="growth"
                            >{translateBillingPlan("growth")}</option
                          >
                          <option
                      ...
                      <div class="grid grid-cols-2 gap-2">
                        ...
                        <input
                          ...
                          name="drawFee"
                          placeholder={t(
                            "saas.tenantsSection.drawFeePlaceholder",
                          )}
                      ...
                      />
                      <label
                        class="label cursor-pointer justify-start gap-3 py-0"
                      >
                        <input
                    ...
                    >
                      ...
                      <input
                        ...
                        placeholder={t(
                          "saas.riskEnvelope.varianceCapPlaceholder",
                        )}
                      />
                      <label
                        class="label cursor-pointer justify-start gap-3 py-0"
                    ...
                    {#if billingCapabilitiesForTenant(item.tenant.id).customerPortal || billingCapabilitiesForTenant(item.tenant.id).paymentMethodSetup}
                      <div class="grid grid-cols-2 gap-2">
                    ...
                    {:else}
                      <p
                        class="rounded-2xl bg-base-200/50 px-3 py-2 text-xs text-slate-500"
                      >
                ...
                {#if tenantLinkChildren(item.tenant.id).length > 0}
                  <div class="mt-4 rounded-2xl bg-base-200/50 p-3">
                    ...
                    <div class="mt-2 space-y-2">
                      {#each tenantLinkChildren(item.tenant.id) as link}
                        <div
                          class="flex items-center justify-between gap-3 rounded-xl border border-base-300 bg-base-100 px-3 py-2 text-sm"
                        >
                          <span
                ...
                <div class="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
                    ...
                    {:else}
                      <div class="mt-3 space-y-2">
                        {#each tenantAgentControls(item.tenant.id) as control}
                          <div
                            class="flex flex-col gap-3 rounded-xl border border-base-300 bg-base-100 px-3 py-3 text-sm md:flex-row md:items-center md:justify-between"
                          >
                            <div>
                              <div class="flex flex-wrap items-center gap-2">
                  ...
                  <form
                    ...
                    class="grid gap-2 rounded-2xl border border-base-300 bg-base-100 p-3"
                  >
                    <input
                      type="hidden"
                      name="tenantId"
                ...
                {#if projectBillingHints(item.tenant.id).length > 0}
                  <div class="mt-4 grid gap-2 md:grid-cols-2">
                    {#each projectBillingHints(item.tenant.id) as billingRun}
                      <div
                        class="rounded-2xl bg-base-200/50 p-3 text-sm text-slate-600"
                      >
  ...
  {#if isProjectsModule}
    <section class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
      <div
        class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between"
      >
        <div>
          <h2 class="text-lg font-semibold">
            {t("saas.projectsSection.title")}
          </h2>
          <p class="text-sm text-slate-500">
      ...
      <div class="mt-4 grid gap-4 xl:grid-cols-2">
        {#each projects as project}
          {@const observability = projectObservabilityByProjectId.get(
            project.id,
          )}
          <article class="rounded-2xl border border-base-300 p-4">
            <div class="flex flex-wrap items-center gap-2">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-3">
              <div class="rounded-2xl bg-base-200/50 p-3 text-sm">
                ...
                <p class="text-xs text-slate-400">
                  ...
                  {formatDate(
                    project.apiRateLimitUsage?.aggregate.burst.resetAt,
                  )}
                </p>
              </div>
              <div class="rounded-2xl bg-base-200/50 p-3 text-sm">
            ...
            {#if observability}
              ...
              >
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-3">
                  <div class="rounded-2xl bg-base-100/80 p-3 text-sm">
                ...
                <div class="mt-4 space-y-2">
                  {#each observability.distribution as bucket}
                    <div
                      class="flex items-center justify-between gap-3 rounded-2xl bg-base-100/80 px-4 py-3 text-sm"
                    >
                      <div>
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        ...
        <div class="mt-4 space-y-2">
          {#each memberships as membership}
            <div
              class="flex items-center justify-between gap-3 rounded-2xl border border-base-300 px-4 py-3 text-sm"
            >
              <div>
                <p class="font-medium text-slate-800">
          ...
          {#each apiKeys as apiKey}
            <div class="rounded-2xl border border-base-300 p-4">
              <div class="flex items-start justify-between gap-4">
                <div>
                  <p class="font-medium text-base-content">{apiKey.label}</p>
                  <p class="mt-1 text-sm text-slate-500">
                  ...
                  {#if apiKey.apiRateLimitUsage}
                    <div class="mt-3 grid grid-cols-3 gap-2 text-xs">
                      <div
                        class="rounded-xl bg-base-200/50 px-2 py-2 text-slate-600"
                      >
  ...
  {#if isBillingModule || isWebhooksModule}
    <section class="grid gap-6 xl:grid-cols-2">
      {#if isBillingModule}
        <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
          <h2 class="text-lg font-semibold">
                ...
                >
                  <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                      ...
                      {billingRun.currency} · {t(
                        "saas.billingRunsSection.draws",
                      )}
                    ...
                    {#if tenantSupportsBillingRunSync(billingRun.tenantId)}
                      ...
                      <div class="flex flex-wrap gap-2">
                        ...
                        >
                          ...
                          />
                          <label
                            class="label cursor-pointer gap-2 py-0 text-xs"
                          >
                            <input
                              class="checkbox checkbox-xs"
                              type="checkbox"
                            ...
                            <span
                              >{t(
                                "saas.billingRunsSection.paidOutOfBand",
                              )}</span
                            >
                          </label>
                          <button class="btn btn-xs">
          ...
          <div class="mt-4 space-y-3">
            {#each webhookEvents as event}
              <div class="rounded-2xl border border-base-300 p-4 text-sm">
                <div class="flex items-center justify-between gap-3">
                  <p class="font-medium text-base-content">{event.eventType}</p>
                  <span class="badge badge-outline"
      ...
      <div class="mt-4 grid gap-3 xl:grid-cols-2">
        ...
        {:else}
          {#each outboundDeliveries as delivery}
            <div
              class="rounded-2xl border border-base-300 p-4 text-sm"
        ...
        <div class="mt-4 space-y-2">
          {#each topUps as topUp}
            ...
            >
              ...
              {#if tenantSupportsTopUpExternalSync(topUp.tenantId) && topUp.status !== "synced" && topUp.status !== "applied"}
              ...
              {:else if tenantSupportsLocalManualCredits(topUp.tenantId) && topUp.status !== "applied" && topUp.status !== "synced"}
                <form method="post" action="?/syncTopUp">
                  <input type="hidden" name="totpCode" value={stepUpCode} />
                  <input type="hidden" name="topUpId" value={topUp.id} />
                  <button class="btn btn-xs btn-secondary">
      ...
      <div class="rounded-3xl border border-base-300 bg-base-100 p-6 shadow">
        ...
        <div class="mt-4 space-y-2">
          {#each recentUsage as usage}
            <div class="rounded-2xl border border-base-300 px-4 py-3 text-sm">
              <p class="font-medium text-base-content">
