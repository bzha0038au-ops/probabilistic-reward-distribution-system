...
type ReportsPageProps = {
  ...
  reportsSubview: PortalReportsSubview | null;
...
export function PortalDashboardReportsPage({
  ...
  const activeSubview = reportsSubview ?? "queue";
      ...
      <section className="grid gap-6">
        <Card className="portal-shell-card-dark portal-fade-up portal-fade-up-delay-2 overflow-hidden rounded-[2rem] text-slate-100">
          <CardHeader className="gap-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Button
                type="button"
                variant="secondary"
                className="rounded-2xl border-white/10 bg-white/10 text-slate-100 hover:bg-white/15"
          ...
          <CardContent className="flex flex-col gap-4">
            {reportsError ? (
              <div className="rounded-[1.5rem] border border-rose-200 bg-rose-50/90 p-4 text-sm text-rose-700">
                {reportsError}
              </div>
            ) : null}

            {hasPendingReportExports ? (
              <div className="portal-banner rounded-[1.5rem] border border-sky-400/20 bg-sky-400/10 p-4 text-sm text-sky-100">
            ...
            {visibleReportExports.length > 0 ? (
              ...
              <div className="rounded-[1.6rem] border border-dashed border-white/15 bg-white/[0.04] p-6 text-sm leading-6 text-slate-400">
                No export jobs are visible for the selected tenant yet. Queue
                one from the form to generate a signed CSV or JSON download.
              </div>
            )}
          </CardContent>
        </Card>
          ...
          {currentTenant ? (
            <>
              ...
              <form
                ...
                onSubmit={handleQueueReportExport}
              >
