...
export async function registerAdminAuditRoutes(protectedRoutes: AppInstance) {
  protectedRoutes.get('/admin/analytics/summary', async (_request, reply) => {
    const { getAnalyticsSummary } = await import('../../../modules/admin/service');
    const summary = await getAnalyticsSummary();
    return sendSuccess(reply, summary);
  });

  protectedRoutes.get('/admin/auth-events', async (request, reply) => {
  ...
  protectedRoutes.get('/admin/admin-actions', async (request, reply) => {
  ...
  protectedRoutes.get('/admin/admin-actions/export', async (request, reply) => {
  ...
  protectedRoutes.get('/admin/auth-events/export', async (request, reply) => {
    const query = toObject(request.query);
    const sort = readStringValue(query, 'sort') === 'asc' ? 'asc' : 'desc';
