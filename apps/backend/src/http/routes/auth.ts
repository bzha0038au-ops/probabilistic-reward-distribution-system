...
const safeRecordAuthEvent = async (
...
const replyWithNotificationError = (
...
const queuePasswordResetNotification = async (payload: {
  userId: number;
  email: string;
}) => {
  return db.transaction(async (tx) => {
...
const queuePhoneVerificationNotification = async (payload: {
  userId: number;
  email: string;
  phone: string;
}) => {
      ...
      if (admin.mfaEnabled) {
        ...
        if (mfaResult.valid && mfaResult.method) {
          ...
          return sendError(
            ...
            401,
            breakGlassCode
              ? "Invalid admin emergency recovery code."
              : totpCode
