...
const updateWalletBalance = async (userId: number, withdrawableBalance: string) => {
  await getDb()
    .update(userWallets)
    .set({
      withdrawableBalance,
...
describeIntegrationSuite("backend prediction market portfolio integration", () => {
  it(
