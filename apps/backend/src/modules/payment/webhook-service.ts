...
const timingSafeHexCompare = (expectedHex: string, provided: string) => {
...
const normalizeSignatureCandidate = (value: string) => {
...
const resolveProviderWebhookSecret = (
  provider: string,
  ...
  const envSecret =
    normalizedProvider === ''
      ? null
      : readString(process.env[`PAYMENT_WEBHOOK_SECRET__${normalizedProvider}`]);
  if (envSecret) {
    return envSecret;
  }

  return readString(Reflect.get(configValue, 'webhookSecret'));
};
...
const derivePaymentWebhookIdentity = (
  provider: string,
  payloadJson: unknown,
  payloadRaw: string,
  ...
  const sources = [dataObject, data, rootObject, root];
  ...
  const providerOrderId = readFirstString(sources, [
    'providerOrderId',
    'providerReference',
    'merchantOrderNo',
    'outTradeNo',
...
export const derivePaymentWebhookTransition = (
  ...
  const providerStatus =
    normalizeToken(
      readFirstString(sources, ['targetStatus', 'status', 'paymentStatus', 'orderStatus']) ??
...
const consumePaymentWebhookEvent = async (event: ClaimedPaymentWebhookEvent) => {
  ...
  switch (transition.action) {
    ...
    case 'withdrawal_mark_provider_submitted': {
      const { markWithdrawalProviderSubmitted } = await import('../withdraw/service');
      updated = await markWithdrawalProviderSubmitted(transition.orderId, review);
      break;
    }
    case 'withdrawal_mark_provider_processing': {
      const { markWithdrawalProviderProcessing } = await import('../withdraw/service');
    ...
    case 'withdrawal_mark_provider_failed': {
      const { markWithdrawalProviderFailed } = await import('../withdraw/service');
...
export async function queuePaymentWebhookEvent(input: {
  ...
  if (!existing) {
    throw new Error('Webhook event dedupe conflict could not be resolved.');
  }
...
export const readPaymentWebhookEventId = (payloadJson: unknown) => {
  ...
  const object = toRecord(Reflect.get(data, 'object'));

  return (
    readString(Reflect.get(payload, 'eventId')) ??
    readString(Reflect.get(payload, 'id')) ??
    readString(Reflect.get(data, 'eventId')) ??
    readString(Reflect.get(data, 'id')) ??
    readString(Reflect.get(object, 'eventId')) ??
    readString(Reflect.get(object, 'id'))
  );
};
