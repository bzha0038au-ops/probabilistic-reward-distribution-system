...
const applySystemConfigDraft = async (
  ...
  if (!Number.isInteger(maxSplitHands) || maxSplitHands < 2 || maxSplitHands > 8) {
    throw unprocessableEntityError(
...
const readChangeRequestAuditSnapshot = async (
  ...
  if (
    ...
    return {
      ...
      targetId: null,
      state: (await toSystemConfigResponse(executor)) as Record<string, unknown>,
    };
  }
...
export async function approveControlChangeRequest(payload: {
  ...
  return db.transaction(async (tx) => {
    ...
    if (request.createdByAdminId === payload.adminId) {
      throw conflictError('The requester cannot approve their own config change.', {
